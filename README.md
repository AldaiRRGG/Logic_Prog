# Computador Adivina el Número

## Introducción

El proyecto consiste en la creación de un sistema interactivo en Python donde el usuario piensa un número entre 0 y 50 y la computadora intenta adivinarlo utilizando estructuras lógicas y repetitivas, el proyecto permitió aplicar los conocimientos adquiridos.



## Descripción del problema

El problema planteado consiste en desarrollar un programa que permita a la computadora adivinar un número pensado por el usuario, para ello, el sistema debe realizar intentos, recibir respuestas del usuario y tomar decisiones hasta encontrar el número correcto, además, el programa debe validar las entradas para evitar errores y garantizar un correcto funcionamiento.



# Objetivos

## Objetivo General

Desarrollar un programa en Python que permita a la computadora adivinar un número pensado por el usuario mediante la aplicación de estructuras lógicas y repetitivas.

## Objetivos Específicos

* Aplicar estructuras condicionales para la toma de decisiones.
* Implementar estructuras repetitivas para controlar el flujo del programa.
* Validar los datos ingresados por el usuario.
* Organizar y documentar adecuadamente el código fuente.
* Utilizar herramientas de desarrollo como Python, Visual Studio Code y GitHub.



## Funcionalidades del Sistema

* Menú principal para iniciar el juego o salir del programa.
* La computadora intenta adivinar el número mediante búsqueda binaria.
* Validación de respuestas ingresadas por el usuario.
* Registro del historial de intentos realizados.
* Contador de partidas jugadas.
* Registro del mejor récord obtenido.
* Mensajes personalizados según la cantidad de intentos realizados.
* Control de errores para evitar respuestas inconsistentes.



## Relación con los Contenidos de la Asignatura

Este proyecto se relaciona directamente con los contenidos estudiados en la asignatura, ya que se aplicaron los siguientes temas:

* Variables.
* Entrada y salida de datos (`input` y `print`).
* Operadores lógicos y relacionales.
* Estructuras condicionales (`if`, `elif`, `else`).
* Estructuras repetitivas (`while`).
* Tuplas.
* Listas.
* Validación de datos.
* Comentarios y documentación del código.



## Explicación del Sistema Desarrollado

El programa inicia mostrando un menú principal con dos opciones: iniciar el juego o salir del sistema, cuando el usuario inicia una partida, debe pensar un número entre 0 y 50, la computadora realiza un primer intento y el usuario responde escribiendo **"mayor"**, **"menor"** o **"correcto"**, dependiendo de la respuesta, el programa ajusta el rango de búsqueda y genera un nuevo intento, este proceso se repite hasta que la computadora adivina correctamente el número.

Además, el sistema almacena el historial de intentos, contabiliza las partidas jugadas y registra el mejor récord obtenido.



## Estructura Lógica del Programa

El sistema utiliza estructuras repetitivas `while` para mantener activo el menú principal y controlar el desarrollo del juego. También emplea estructuras condicionales `if`, `elif` y `else` para validar las respuestas ingresadas por el usuario y determinar el comportamiento del programa en cada situación.



## Organización del Código

El código fue desarrollado de manera secuencial y estructurada, incorporando comentarios en las secciones más importantes para facilitar su comprensión y mantenimiento, esto permite que cualquier usuario pueda entender fácilmente el funcionamiento del sistema.



## Uso de Herramientas de Desarrollo

Para el desarrollo del proyecto se utilizaron las siguientes herramientas:

* Python
* Visual Studio Code
* GitHub
* OBS Studio



## Cronograma de Aprendizaje y Desarrollo del Proyecto

| Semana   | Actividad                                                                              |
| -------- | -------------------------------------------------------------------------------------- |
| Semana 1 | Introducción a la lógica de programación y análisis de problemas.                      |
| Semana 2 | Uso de variables, operadores y entrada/salida de datos, diagramas.                     |
| Semana 3 | Aplicación de estructuras condicionales (`if`, `elif`, `else`).                        |
| Semana 4 | Implementación de estructuras repetitivas (`while`).                                   |
| Semana 5 | Condicionales, operadores racionales y lógicos.                                        |
| Semana 6 | Desarrollo del avance inicial del programa, implementación de Bucles, `While` y `For`. |
| Semana 7 | Implementación de tuplas y mejoras del sistema final.                                  |



## Reflexión sobre el Impacto de la Tecnología

La tecnologia en la actualidad desempeña un papel fundamental en la solucion de problemas mediante desarrollo de software, en este proyecto se pudo evidenciar como la programacion nos puede ayduar a realizar desde la cosa mas minima hasta algo mucho mas grande, tambien el uso de herramientas tecnologicas facilita el aprendizaje y contriubuye al desarrollo de soluciones.


## Autor

**Aldair Guallimba**


